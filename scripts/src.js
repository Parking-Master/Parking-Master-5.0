      ...
      function animate(now) {
        ...
        if (settings.performance.replay) {
          if (utils.data.showReplay) {
            const data = utils.data.replayFrames[utils.data.currentReplayFrame];
            if (data) {
              vehicle.body.position.copy(data[0]);
              vehicle.body.quaternion.copy(data[1]);
              vehicle.body.velocity.copy(data[2]);
              vehicle.body.angularVelocity.copy(data[3]);
              utils.data.currentReplayFrame++;
              camera.position.set(-25, 5, 15);
              camera.lookAt(vehicle.mesh.position);
              camera.fov = Math.max(30 - (vehicle.mesh.position.distanceTo(camera.position) / 4), 15);
              camera.updateProjectionMatrix();
            } else {
              utils.data.showReplay = false;
              utils.data.currentReplayFrame = 0;
            }
          } else {
            utils.data.replayFrames.push([vehicle.body.position.clone(), vehicle.body.quaternion.clone(), vehicle.body.velocity.clone(), vehicle.body.angularVelocity.clone()]);
          }
        }
      ...
      utils = {
        ...
        game: {
          ...
          loadLevel: function(level) {
            ...
            if (level != utils.game.currentLevel && settings.performance.replay && utils.data.replayFrames.length > 0 && level != 1) {
              let clips = JSON.parse(localStorage["replay-clips"] || "{}");
              let entry = {
              ...
              clips["Level " + utils.game.currentLevel] = btoa(JSON.stringify(entry));
              // localStorage.setItem("replay-clips", JSON.stringify(clips));
